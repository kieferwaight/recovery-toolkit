...
grep -Fq 'create_runtime_key' "${repo_dir}/bin/provision-luks" || fail 'shared key generation missing'
grep -Fq 'cleanup_runtime_secret' "${repo_dir}/bin/provision-luks" || fail 'shared key cleanup missing'
grep -Fq 'audit_vault_path' "${repo_dir}/bin/provision-luks" || fail 'vault path helper missing'
grep -Fq 'luksHeaderBackup' "${repo_dir}/bin/provision-luks" || fail 'header backup missing'
grep -Fq 'secret_sha256' "${repo_dir}/bin/provision-luks" || fail 'secret checksum helper missing'
